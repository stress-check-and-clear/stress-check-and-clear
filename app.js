...
const WEB_APP_URL = "https://script.google.com/macros/s/AKfycbzx2FUnp3cH4-QGijHBV3rWZMe3j32lm1KzFcp4Lz7iCJHxNVDe9qbWsPbqggox2-SX/exec";
...
const state = {
  ...
  get gameFinished() {
    return this.gameFocusFinished || this.gameMemoryFinished;
  },
};
...
// fetch helper: text/plain + retry + timeout, non-blocking
async function fire(action, payload = {}, { retry = 1, timeoutMs = 9000 } = {}) {
  ...
  try {
    ...
    showToast("เครือข่ายช้า/ขัดข้อง — จะลองส่งใหม่อัตโนมัติครั้งถัดไป");
    return null;
  }
}
...
async function ensureUid() {
  if (state.completed) {
    showToast("คุณทำแบบประเมินครบทุกส่วนแล้ว โปรดเริ่มใหม่หากต้องการทำอีกครั้ง");
    setStep(10);
    return { uid: state.uid, user_id: state.user_id };
  }
  if (state.uid && state.user_id) {
    const resp = await fire("init", { uid: state.uid });
    if (resp && resp.ok && !resp.completed) {
      return { uid: state.uid, user_id: state.user_id };
    }
    if (resp && resp.completed) {
      state.completed = true;
      localStorage.setItem("spst_completed", "true");
      showToast("คุณทำแบบประเมินครบทุกส่วนแล้ว โปรดเริ่มใหม่หากต้องการทำอีกครั้ง");
      setStep(10);
      return { uid: state.uid, user_id: state.user_id };
    }
  }
  const resp = await fire("init", { uid: state.uid || null });
  if (resp && resp.ok) {
    state.uid = resp.uid;
    ...
    localStorage.setItem("spst_completed", resp.completed.toString());
    return { uid: state.uid, user_id: state.user_id };
  }
...
document.getElementById("startBtn").addEventListener("click", async () => {
  try {
    ...
    showToast("เริ่มต้นระบบไม่สำเร็จ ลองใหม่อีกครั้ง");
  }
});
...
document.getElementById("consentYes").addEventListener("click", async () => {
  if (state.completed) {
    showToast("คุณทำแบบประเมินครบทุกส่วนแล้ว โปรดเริ่มใหม่");
    setStep(10);
    return;
  }
  state.consent = true;
  setStep(3);
  ensureUid().then(({ uid }) => fire("saveConsent", { uid, consent: true }));
});
...
document.getElementById("consentNo").addEventListener("click", async () => {
  ...
  state.consent = false;
  setStep(5);
  ensureUid().then(({ uid }) => fire("saveConsent", { uid, consent: false }));
});
    ...
    <div class="scale" role="radiogroup" aria-label="ระดับความเครียด ข้อที่ ${idx}">
      ${[
        ...
        .map(
          (lab, valIdx) => `
        <label>
          <input type="radio" name="q${idx}" value="${valIdx + 1}" />
          <span>${lab}</span>
        </label>`
        )
        .join("")}
...
document.getElementById("calcBtn").addEventListener("click", async () => {
    ...
    sum >= 62
      ? `<div class="helpbox" style="margin-top:12px">
           ...
           <ul style="margin:8px 0 0 18px">
             ...
             <li><strong>1669</strong> (การแพทย์ฉุกเฉิน)</li>
  ...
  scoreBox.innerHTML = `
    ...
    <div class="muted" style="margin-top:6px;white-space:pre-line">${detailHtml}</div>
    ${severeHelp}
    <div class="btns" style="margin-top:10px">
      <button class="primary" id="toPart2">ไปส่วนที่ 2</button>
    </div>
  `;
...
choiceContainer.querySelectorAll(".chip").forEach((ch) => {
  ch.addEventListener("click", () => {
    ...
    ensureUid().then(({ uid }) => fire("saveChoice", { uid, choice }));
  });
});
...
function renderMethod() {
  ...
  if (c === "music") {
    ...
    methodPane.innerHTML += `
      ...
      ${[
        "https://www.youtube.com/embed/dhFt7eURm78?rel=0",
        "https://www.youtube.com/embed/HAzZH6wccew?rel=0",
        "https://www.youtube.com/embed/hlWiI4xVXKY?rel=0",
      ]
        .map(
          (src) => `
        <div style="position:relative;padding-top:56.25%;border-radius:12px;overflow:hidden;border:1px solid #e5e7eb;margin-bottom:12px">
          <iframe src="${src}" title="เพลงผ่อนคลาย" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen style="position:absolute;inset:0;width:100%;height:100%;border:0">
          </iframe>
        </div>`
        )
        .join("")}
  ...
  if (c === "video") {
    ...
    methodPane.innerHTML += `
      ...
      <div style="position:relative;padding-top:56.25%;border-radius:12px;overflow:hidden;border:1px solid #e5e7eb">
        <iframe title="relax-video" src="https://www.youtube.com/embed/2OEL4P1Rz04?rel=0" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen style="position:absolute;inset:0;width:100%;height:100%;border:0"></iframe>
      </div>`;
    return;
  }
  if (c === "game") {
...
function setupGame() {
  ...
  function stop() {
    ...
    openCongratsModal();
    manageNextButtonInStep7();
    ensureUid().then(({ uid }) => fire("saveGame", { uid, game: "focus", score }));
  }
  ...
  function check() {
    ...
    if (a && b && a.dataset.icon === b.dataset.icon) {
      ...
      if (matched === cards.length) {
        ...
        openCongratsModal();
        manageNextButtonInStep7();
        ensureUid().then(({ uid }) => fire("saveGame", { uid, game: "memory", score: matched / 2 }));
      }
    } else {
      if (a) {
